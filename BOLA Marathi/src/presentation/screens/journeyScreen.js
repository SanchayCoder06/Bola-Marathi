 ...
 * Presentation Layer (RPG Layer (RPG Exploration & World Map System)
 */

import { AppState } from '../../application/state/appState.js';
import { GameEngine } from '../../core/engines/gameEngine.js';
import { Progress & World Map System)
 */

import { AppState } from '../../application/state/appState.js';
import { GameEngine } from '../../application/state/appState.js';
import { GameEngine } from '../../core/engines/gameEngine.js';
...
export const JourneyScreen = {
  async render(container) {
    ...
    const currentCityId = state.rpg.currentCity || 'pune';
    const cities = GameEngine.getCityMetadata();

    // Fetch shared data for dynamic values
    const conversations = await DBService.getAll('dialogueTrees');
    const dictionary = await DBService.getAll('dictionary');

    // Helper to compute quest progress for a city
    const getQuestProgress = (cityId) => {
      const cityConvos = conversations.filter(c => c.id.startsWith(cityId + '_'));
      const total = cityConvos.length;
      const completed = cityConvos.filter(c => state.rpg.completedScenarios.includes(c.id)).length;
      return { completed, total };
    };

    // Helper to compute vocabulary stats
    const getVocabStats = () => {
      const total = dictionary.length;
      const unlocked = dictionary.filter(entry => entry.unlocked === true).length;
      return { total, unlocked };
    };

    // Helper to compute NPC relationship percent for a city
    const getNpcRelPercent = (cityId) => {
      const city = cities[cityId];
      if (!city) return 0;
      let total = 0;
      let max = 0;
      for (const landmark of city.landmarks) {
        const nickname = landmark.npcName.split(' ').pop().toLowerCase();
        const points = state.rpg.npcRelationships[nickname] || 0;
        total += points;
        max += 50;
      }
    ...
    // Helper to compute achievements percent for a city (souvenirs + recipes)
    const getAchievementPercent = (cityId) => {
      const city = cities[cityId];
      if (!city) return 0;
      const totalSouvenirs = city.souvenirs?.length || 0;
      const totalRecipes = city.recipes?.length || 0;
      const total = totalSouvenirs + totalRecipes;
      if (total === 0) return 0;
      const unlockedSouvenirs = state.rpg.unlockedSouvenirs.filter(id => city.souvenirs.some(s => s.id === id)).length;
      const unlockedRecipes = state.rpg.unlockedRecipes.filter(id => city.recipes.some(r => r.id === id)).length;
      const unlocked = unlockedSouvenirs + unlockedRecipes;
      return Math.round((unlocked / total) * 100);
    };

    // Helper to get story completion percent (using quest completion as placeholder)
    const getStoryCompletionPercent = (cityId) => {
      const { completed, total } = getQuestProgress(cityId);
      return total === 0 ? 0 : Math.round((completed / total) * 100);
    };

    // Helper to get district explore percent (placeholder - not implemented)
    const getDistrictExplorePercent = (cityId) => 0;

    // Helper to get festival badge percent (placeholder - not implemented)
    const getFestivalBadgePercent = (cityId) => 0;

    // Compute data for current city
    const currentCity = cities[currentCityId];
    const questProgress = getQuestProgress(currentCityId);
    const npcCount = currentCity.landmarks.length;
    const vocabStats = getVocabStats();
    const cityProgressPercent = questProgress.total > 0 ? Math.round((questProgress.completed / questProgress.total) * 100) : 0;
    const storyCompletionPercent = getStoryCompletionPercent(currentCityId);
    const districtExplorePercent = getDistrictExplorePercent(currentCityId);
    const npcRelationshipPercent = getNpcRelPercent(currentCityId);
    const vocabScrollPercent = vocabStats.total > 0 ? Math.round((vocabStats.unlocked / vocabStats.total) * 100) : 0;
    const achievementsPercent = getAchievementPercent(currentCityId);
    const festivalBadgePercent = getFestivalBadgePercent(currentCityId);

    // Package data for passing to renderers
    const cityData = {
      questProgress,
      npcCount,
      vocabStats,
      ...
      districtExplorePercent,
      npcRelationshipPercent,
      vocabScrollPercent,
      achievementsPercent,
      festivalBadgePercent
    };
    ...
    if (activeLandmark && !activeLandmark.endsWith('-district')) {
      if (activeLandmark === 'pune-restaurant' && JourneyScreen.previewStoryId === 'pune_restaurant') {
        _renderStoryPreview(container, currentCityId);
      } else {
        _renderDialogueSimulator(container, activeLandmark, currentCityId);
      }
      return;
    }

    // 2. Regional Animated World Map View
    const hash = window.location.hash;
    if (hash.startsWith('#journey/') && currentCityId) {
      const parts = hash.split('/');
      if (parts.length > 2) {
        const districtId = parts[2];
        _renderDistrictExplore(container, currentCityId, districtId);
      } else {
        _renderCityLandingPage(container, currentCityId, cityData);
      }
      return;
    }

    // 3. Regional Animated World Map View (default)
    _renderRegionalMap(container, rep.level, currentCityId, cityData);
  },
  previewStoryId: null
};
...
// ============================================================
function _renderRegionalMap(container, playerLevel, currentCityId, cityData) {
  const cities = GameEngine.getCityMetadata();
  ...
  container.querySelectorAll('.map-pin-anchor').forEach(pin => {
    pin.addEventListener('click', () => {
      ...
      _openCityBottomSheet(cityId, playerLevel, cityData);
    });
  });

  function _openCityBottomSheet(cityId, playerLevel, cityData) {
    const city = cities[cityId];
    if (!city) return;
      ...
      sheet.innerHTML = `
        ...
        <div style="background: rgba(0,0,0,0.2); border-radius: 20px; padding: 16px; border: 1px solid rgba(255,255,255,0.03); margin-bottom: 24px; display: grid; grid-template-columns: 1fr 1fr; gap: 12px; font-size: 0.78rem;">
          <div>
            ...
            <div style="color: #fff; font-weight: bold; margin-top: 2px;">${cityData.questProgress.completed} / ${cityData.questProgress.total} Quests</div>
          </div>
          <div>
            <div style="color: rgba(255,255,255,0.4); text-transform: uppercase; font-size: 0.65rem; font-weight: bold;">NPCs Inhabiting</div>
            <div style="color: #fff; font-weight: bold; margin-top: 2px;">${cityData.npcCount} Characters</div>
          </div>
          <div>
            <div style="color: rgba(255,255,255,0.4); text-transform: uppercase; font-size: 0.65rem; font-weight: bold;">Vocabulary Unlocked</div>
            <div style="color: #FF9F1C; font-weight: bold; margin-top: 2px;">${cityData.vocabStats.unlocked} Words</div>
          </div>
          <div>
            <div style="color: rgba(255,255,255,0.4); text-transform: uppercase; font-size: 0.65rem; font-weight: bold;">City Progress</div>
            <div style="color: #2ED573; font-weight: bold; margin-top: 2px;">${cityData.cityProgressPercent}%</div>
          </div>
        </div>
      ...
      sheet.innerHTML = `
        ...
        <div style="background: rgba(0,0,0,0.2); border-radius: 20px; padding: 16px; border: 1px solid rgba(255,255,255,0.03); margin-bottom: 24px; font-size: 0.8rem;">
          ...
          <div style="margin-top: 12px; display: flex; justify-content: space-between; font-size: 0.72rem; color: rgba(255,255,255,0.45);">
            <span>📖 ${cityData.questProgress.total - cityData.questProgress.completed} Quests waiting</span>
            <span>🕒 ~${Math.max(0, ((cityData.questProgress.total - cityData.questProgress.completed) * 2) / 60).toFixed(1)} Hours playtime</span>
          </div>
        </div>
...
// ============================================================
function _renderCityLandingPage(container, cityId, cityData) {
  const state = AppState.getState();
  const cities = GameEngine.getCityMetadata();
  const city = cities[cityId];
  if (!city) {
    ...
    <div class="screen active" id="screen-city-landing" style="display: flex; flex-direction: column; gap: 26px; background: #090B15; padding: 20px 20px 85px; font-family: 'Outfit', sans-serif;">
      ...
      <div class="luxury-card" style="padding: 0; overflow: hidden; position: relative;">
        ...
        <div style="padding: 20px; margin-top: -30px; position: relative; z-index: 5;">
          ...
          <div style="border-top: 1px solid rgba(255,25500px; border-top: 1px solid rgba(255,255,255,0.05); padding-top: 14px; display: flex; justify-content: space-between; align-items: center;">
            <div>
...
function _triggerVoiceGradingOverlay(container, landmarkId, cityId, optionIndex, targetText) {
  ...
  overlay.innerHTML = `
    ...
    <button id="btnCancelGrading" style="width: 100%; border: 1px solid rgba(255,255,255,0.06); background: rgba(0,0,0,0.25); color: #fff; padding: 12px; border-radius: 12px; font-weight: bold; cursor: pointer;">
      Cancel
    </button>
...
// Assuming UI is defined elsewhere (global or imported)
var UI = {
